...
class IoTCClient:
    ...
    def _on_message(self, topic, message):
        ...
        if topic.startswith(HubTopics.PROPERTIES):
            # desired properties
            self._logger.info("Received desired property message: {}".format(message))
            ...
            params = topic.split(
                "devices/{}/messages/devicebound/".format(self._device_id), 1
            )[1].split("&")
            for param in params:
                p = param.split("=")
                if p[0] == "method-name":
    ...
    def send_telemetry(self, payload, properties=None):
        ...
        if properties is not None:
            for prop in properties:
                topic += "{}={}&".format(
                    encode_uri_component(prop), encode_uri_component(properties[prop])
                )
    ...
    def _cmd_ack(self, command: Command):
        ...
        self._mqtt_client.publish(
            "$iothub/methods/res/{}/?$rid={}".format(200, command.request_id).encode(
                "utf-8"
            ),
            "",
        )
